    ...
    methods: {
        ...
        onchange() {
            ...
            if(this.checkedList.length>0){
                this.orderMatchCouponList.filter(item => this.checkedList.indexOf(item.id)!=-1).forEach(item => this.finalPrice= ((this.finalPrice-item.discountMoney)*item.discount).toFixed(1))
                //this.orderMatchCouponList.filter(item => this.checkedList.indexOf(item.id)!=-1&&(item.couponType==1||item.couponType==2)).forEach(item => this.finalPrice= (this.finalPrice-item.discount_money)*item.discount)
            }else{
